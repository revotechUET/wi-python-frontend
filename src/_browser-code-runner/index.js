...
export function service(mime, config, projectApi) {
	...
	const render = code => {
		let runningProcesses = JSON.parse(localStorage.getItem('running-pid') || '[]');
		runningProcesses.push(code.data.pid)
		localStorage.setItem('running-pid', JSON.stringify(runningProcesses));
		return "========= START ===========";
		/*
				let renderVal = ''
